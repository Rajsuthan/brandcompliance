...
export const MediaItem: React.FC<MediaItemProps> = ({
  ...
  return (
    <div className="border border-zinc-700 rounded-lg overflow-hidden w-full shadow-lg shadow-black/20 hover-lift mb-4">
      <div className="aspect-video bg-zinc-900 flex items-center justify-center overflow-hidden">
        {isVideo ? (
          ...
          <img src={preview} alt={file.name} className="w-full h-auto object-contain" />
        )}
      </div>
      <div className="p-3 bg-zinc-800 flex flex-col gap-2">
        ...
        <div className="w-full">
          <label
            htmlFor={`brandName-${file.name}`}
            className="block text-xs font-medium text-zinc-400 mb-1"
          >
            Brand Name (required)
          </label>
          <input
            id={`brandName-${file.name}`}
            type="text"
            value={brandName}
