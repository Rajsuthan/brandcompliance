...
export const MultiFileUpload: React.FC<MultiFileUploadProps> = ({ onFilesReady }) => {
  const [mediaFiles, setMediaFiles] = useState<MediaFile[]>([]);
  
  const handleFileSelect = (file: File) => {
    // Check if file already exists in the list
    if (mediaFiles.some(item => item.file.name === file.name && item.file.size === file.size)) {
  ...
  const handleBrandNameChange = (id: string, brandName: string) => {
    setMediaFiles(prev =>
      prev.map(file =>
        file.id === id ? { ...file, brandName } : file
      )
    );
  };
    ...
    <div className="flex flex-col space-y-6">
      ...
      {mediaFiles.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-md font-medium text-white">Uploaded Files ({mediaFiles.length})</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {mediaFiles.map(mediaFile => (
              <MediaItem
                ...
                onBrandNameChange={(brandName) => handleBrandNameChange(mediaFile.id, brandName)}
          ...
          <Button
            ...
            disabled={mediaFiles.length === 0 || !mediaFiles.every(file => file.brandName.trim() !== '')}
