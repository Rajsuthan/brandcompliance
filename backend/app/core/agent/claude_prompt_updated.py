...
## ⚠️ CRITICAL: FINAL COMPLETION FORMAT ⚠️

Here is EXACTLY how you must format your final completion. Copy this format precisely:

```
After thorough analysis, I've gathered sufficient information to provide a comprehensive compliance assessment.

I'll now use the attempt_completion tool to submit my final analysis:

<attempt_completion>
<r>
## Compliance Analysis for Brand Promotional Material

### Executive Summary
❌ Non-Compliance: This material demonstrates non-compliance with key brand standards. The primary issues include blurred logo reproduction, incorrect color usage, and improper typography.

### Methodology
I conducted a systematic analysis by:
1. Examining logo clarity using check_image_clarity
2. Analyzing color schemes using get_region_color_scheme
3. Verifying typography against guidelines
4. Checking element placement and spacing
5. Cross-referencing all findings with official brand guidelines

### Detailed Analysis

#### Logo Issues
- The logo appears blurred with clarity scores below the minimum requirement
- Edge definition is particularly poor
- This violates the guideline: "Logos must always be reproduced with maximum clarity"

#### Color Palette Issues
- Background uses incorrect RGB values that deviate from the official palette
- Color deviation exceeds the maximum tolerance
- This creates inconsistent brand representation

#### Typography Compliance
- Correct font is used for all text elements
- However, letter spacing is below minimum requirements
- Text contrast ratio is below the required minimum for accessibility

### Recommendations
1. **Logo Clarity**: Replace all instances of the logo with the official vector version
2. **Color Correction**: Adjust colors to match official brand palette
3. **Typography Adjustments**: Increase letter spacing to meet minimum requirements
4. **Contrast Enhancement**: Increase text contrast ratio for better readability
5. **Quality Control**: Implement a pre-release compliance check

These corrections are necessary before this material can be approved for public release.
</r>
...
YOU MUST USE THIS EXACT XML FORMAT FOR THE FINAL COMPLETION.

The attempt_completion tool MUST be formatted with:
1. Opening tag: `<attempt_completion>`
2. The `<r>` tag containing your full analysis
3. The `<tool_name>attempt_completion</tool_name>` parameter
4. The `<task_detail>description</task_detail>` parameter
5. Closing tag: `</attempt_completion>`
